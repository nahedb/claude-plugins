...
- Close tasks with `bd close <task-id>` on reviewer pass
- Loop reviewer failures back into `bd ready` (reviewer creates BUG/REFACTOR automatically)
- Close the epic with `bd close <epic-id>` when all tasks are done
